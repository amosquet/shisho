...
def extract_grounding_sources(
    responses: Any | list[Any],
) -> list[tuple[str, str]]:
    """
    Extract web sources (title, uri) from Gemini GroundingMetadata across one or more responses.

    Returns:
        List of tuples (title, uri) deduplicated by uri in order of appearance.
    """
    ...
    for resp in resp_list:
        ...
        for cand in candidates:
            ...
            for chunk in chunks:
                ...
                clean_title = (
                    raw_title.replace("[", "").replace("]", "").strip() or "Source"
                )
                sources.append((clean_title, uri))

    return sources


def format_grounding_sources(
    sources: list[tuple[str, str]], max_sources: int = 5
) -> str:
    """
    Format a list of (title, uri) tuples into a Discord-ready markdown section with
    embed suppression (<uri>).
    """
    if not sources:
        return ""
    selected = sources[:max_sources]
    lines = [f"- [{title}](<{uri}>)" for title, uri in selected]
    return "\n\n**Sources:**\n" + "\n".join(lines)
